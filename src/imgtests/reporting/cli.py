...
if TYPE_CHECKING:
    ...
    from imgtests.reporting.distro_comparison_export import MetricDefinition
...
DISTRO_COMPARISON_METRICS_COMMAND: Final = "comparison-metrics"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    metrics_parser = subparsers.add_parser(
        ...
        help="print inferred comparison metrics as a table",
    )
    add_distro_comparison_metrics_arguments(metrics_parser)
    metrics_parser.set_defaults(command=DISTRO_COMPARISON_METRICS_COMMAND)
...
def add_distro_comparison_metrics_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "input",
        type=Path,
        help="Existing report.xlsx generated by the database export.",
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Experiment IDs to inspect. With two IDs, one Poky/SUSE group is used. "
            "By default all comparable Poky/SUSE pairs are used."
        ),
    )


def format_metric_definitions_table(metric_definitions: Sequence[MetricDefinition]) -> str:
    table = PrettyTable()
    table.field_names = [
        "metric_name",
        "base_metric_name",
    ...
    table.align = "l"
    table.max_width["description"] = 90
    ...
    for definition in metric_definitions:
        table.add_row(
            [
                ...
                format_higher_is_better(definition.higher_is_better),
...
def format_higher_is_better(value: bool | None) -> str:
    if value is None:
