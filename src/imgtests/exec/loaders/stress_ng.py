...
def build_stress_retry_args(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if "cpu" in retry_args:
        cpu_value = __safe_int(retry_args["cpu"], default=1)
        retry_args["cpu"] = 1 if cpu_value == 0 else max(1, min(cpu_value, 2))

    if "sock" in retry_args:
        retry_args["sock"] = __half_positive_int(retry_args["sock"], fallback=1)
...
def __half_positive_int(value: Any, fallback: int = 1) -> int:
    try:
