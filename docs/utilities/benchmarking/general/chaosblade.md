...
2. [Memory](https://deepwiki.com/chaosblade-io/chaosblade-for-deepwiki/3.2-memory-experiments)
	* Имитируют сценарии нехватки памяти за счет выделения и использования памяти
	* Доступные действия:
	...
	* Флаги определения размера памяти:
		...
		* Если есть `--mem-percent`, то оно имеет приоритет. Если есть только `--reserve`, то используется `--reserve`. В противном случае происходит неявное вычисление размера.
	* Флаги:
...
6. [File System](https://deepwiki.com/chaosblade-io/chaosblade-for-deepwiki/3.6-file-system-experiments) - моделирование сбоев файловой системы путем манипулирования файлами и каталогами.
	...
	* Флаги для `append`:
		...
		* `--interval` - интервал между добавлениями (в секундах)
		...
		* `--delete-file` - удаление файла при завершении работы эксперимента. Переопределяет `--enable-backup`
	* Флаги для `chmod`:
		* `--filepath` - путь к файлу
		* `--mark` - права доступа
	* Флаги для `add`:
